...
--LOAD DATA LOCAL INFILE '.\\src\\main\\resources\\userlog.csv'
--INTO TABLE userlog
--FIELDS TERMINATED BY ','
--ENCLOSED BY '"'
--LINES TERMINATED BY '\r\n'
...
--    departure_time = STR_TO_DATE(CONCAT(@date, ' ', LEFT(@departure_time, 2), ':', RIGHT(@departure_time, 2)), '%Y%m%d %H:%i'),
--    arrival_time = DATE_ADD(STR_TO_DATE(CONCAT(@date, ' ', LEFT(@departure_time, 2), ':', RIGHT(@departure_time, 2)), '%Y%m%d %H:%i'), INTERVAL @use_time MINUTE),
--    use_time = @use_time,
